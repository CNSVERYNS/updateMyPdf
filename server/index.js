...
app.post('/api/storage/share', async (request, response) => {
  ...
  try {
    ...
    response.status(error?.status || 500).json({ error: error?.status === 401 ? error.message : 'Payla\u015f\u0131m ba\u011flant\u0131s\u0131 olu\u015fturulamad\u0131.' })
  }
})
...
const formatAiCommandError = (error) => {
  ...
  const userError = googleError
    ...
    : status === 402
      ...
      : rateLimited
        ? `AI servisi \u015fu anda yo\u011fun${pageSuffix}. Sistem tekrar denedi ancak bu i\u015flem tamamlanamad\u0131; l\u00fctfen biraz sonra tekrar dene.`
        : timedOut
          ? `AI i\u015flemi zaman a\u015f\u0131m\u0131na u\u011frad\u0131${pageSuffix}. Daha k\u00fc\u00e7\u00fck bir sayfa aral\u0131\u011f\u0131 denemelisin.`
          : error?.cause
            ? `\u00c7eviri${pageSuffix} tamamlanamad\u0131. Sistem ayn\u0131 sayfay\u0131 tekrar denedi; kaynak PDF metnini kontrol edip tekrar dene.`
            : `AI komutu i\u015flenirken bir hata olu\u015ftu${process.env.NODE_ENV === 'production' ? '' : ` (${rootError?.message || 'unknown error'})`}`
...
const isRetryableAiError = (error) => {
  ...
  return status === 0 || status === 408 || status === 409 || status === 429 || status >= 500 || ['ETIMEDOUT', 'ECONNRESET', 'ECONNABORTED'].includes(error?.code) || ['APIConnectionTimeoutError', 'APIConnectionError', 'InternalServerError', 'RateLimitError'].includes(error?.name)
}
...
app.get('/api/ai/command/:jobId', (request, response) => {
  ...
  if (!job || !accessToken || accessToken !== job.accessToken) return response.status(404).json({ error: 'AI i\u015flemi bulunamad\u0131 veya s\u00fcresi doldu.' })
...
app.post('/api/ai/command', upload.fields([{ name: 'file', maxCount: 1 }, { name: 'image', maxCount: 1 }]), async (request, response) => {
  ...
  if (translationMode) {
    try {
      ...
      preExtractedTranslationPageCount = extractedPages.length
      preExtractedTranslationPages = extractedPages.filter((page) => page.text)
...
Automatically identify the source language from the extracted PDF text; the user must never be required to name the source language. Detect mixed-language pages independently and translate every non-target portion. Translate every piece of prose on every page. Never summarize, shorten, paraphrase, omit sentences, omit paragraphs, or say that translation is unnecessary. Preserve names, numbers, dates, headings, labels, punctuation, and the order of the content. Treat mixed-language text, Lorem Ipsum, pseudo-Latin, filler text, and placeholder prose as non-target content; do not leave it unchanged just because it is not normal modern prose. Translate or rewrite that content into coherent ${targetLanguageHint} while keeping its sentence and paragraph coverage. The source text is grouped by page and its line breaks represent the original visual layout. For each page, return exactly one translate action. The replacement must contain the COMPLETE translation of that page and must contain exactly the same number of non-empty lines as the source page. Do not merge multiple source lines into one line, split one source line into multiple lines, or add commentary. If a page is already partly in ${targetLanguageHint}, keep the existing target-language text but translate all other text. Never skip the final page. Return only the compact translation JSON schema fields; do not return explanations, summaries, or null fields.`
    ...
    if (translationPlanNeedsRetry(plan)) {
      ...
      const translatePage = async (page) => {
        ...
        const finalError = new Error(`Translation for page ${page.page} failed after 3 attempts.`)
        finalError.status = lastError?.status || lastError?.statusCode
        finalError.cause = lastError
        throw finalError
      }
      const workerCount = Math.min(2, extractedTranslationPages.length)
  ...
  if (largeTranslation) {
    ...
    return response.status(202).json({ status: 'processing', jobId, jobToken: accessToken, pageCount: preExtractedTranslationPageCount || preExtractedTranslationPages?.length || 0 })
  }
